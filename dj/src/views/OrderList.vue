                    ...
                    <div class="tbodys" v-for="(item,index) in list " :key="index">
                        <div class="td">
                            <div class="order-info">
                                <span>2020-05-16 10:11:22</span>
                                <span>订单编号：111111111</span>
                            </div>
                            <div class="detail clearfix">
                                ...
                                <div class="num fl">25元/局 x 1</div>
                                <div class="price fl">
                                    <div id="cell">
                                        <p v-if="item.type==1">待付款</p>
                                        <p v-if="item.type==2||item.type==3">已付款</p>
                                        <p>25元</p>
                                    </div>
                                </div>
                                <div class="status fl">
                                    <span v-if="item.type==1">未服务</span>
                                    <span v-if="item.type==2">服务中</span>
                                    <span v-if="item.type==3">已完成</span>
                                </div>
                                <div class="do fl unselect">
                                    <div class="type type1">
                                        <div class="refund btn btnclick" v-if="item.type==1">去付款</div>
                                        <div class="refund btn btnclick" v-if="item.type==2">完成订单</div>
                                        <div class="contact btn btnclick">联系陪玩</div>
                                        <div class="contact btn btnclick" v-if="item.type==3" @click="$router.openPage('/refund')">申请退款</div>

                                        <div
                                            class="contact btn btnclick"
                                            v-if="item.type==1||item.type==2"
                                        >取消订单</div>
                                        <div class="contact btn btnclick" v-if="item.type==3" @click="$router.openPage('/comments')">立即评论</div>
                                    </div>
                                </div>
                            </div>
                        </div>
